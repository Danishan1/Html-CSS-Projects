...
export const isAuth = async (req, res, next) => {
    try {
        ...
        if (!req.session || !req.session.authToken) {
            return res.status(401).json({ unqueCode: "0000A", genericCode: "RTAD401", message: "Unauthorized. Please log in." });
        }
        ...
        if (rows.length === 0) {
            return res.status(401).json({ unqueCode: "0000B", genericCode: "RTAD401", message: "Unauthorized. Please log in again." });
        }
        ...
        res.status(500).json({ unqueCode: "0000C", genericCode: "RTAD500", message: "Internal server error" });
    }
};
